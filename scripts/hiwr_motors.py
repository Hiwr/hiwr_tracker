...
class HiwrTracker():
    def __init__(self):
        ...
        rospy.loginfo("namespace is "+namespace)
        self.joints = [rospy.get_param("~pan_name", ''), rospy.get_param("~tilt_name", '')]#rospy.get_param(namespace + '/joints', '')

        # What are the names of the pan and tilt joint in the list of dynamixels?
        self.head_pan_joint = rospy.get_param("~pan_name", 'pan_joint') #'head_pan_joint'
        self.head_tilt_joint = rospy.get_param("~tilt_name", 'tilt_joint')  #'head_tilt_joint'
        ...
        self.lead_target_angle = rospy.get_param('~lead_target_angle', 0.8)
        ...
        self.max_pan = rospy.get_param("~max_pan", radians(300))
        self.min_pan = rospy.get_param("~min_pan", radians(-300))
        self.max_tilt = rospy.get_param("~max_tilt", radians(300))
        self.min_tilt = rospy.get_param("~min_tilt", radians(-300))
        ...
        rospy.wait_for_message('/uvc_cam_node/roi', RegionOfInterest)
        ...
        rospy.Subscriber('/uvc_cam_node/roi', RegionOfInterest, self.set_joint_cmd)
        ...
        while not rospy.is_shutdown():
            ...
            if self.target_lost_count > self.max_target_lost_count:
                #self.center_head_servos()
                rospy.loginfo("lost target -> recentering")
            else:
                try:
    ...
    def init_servos(self):
        ...
        rospy.loginfo("Waiting for joint controllers services... HI")
...
means. So we define a very small speed value to represent zero speed. """
    ...
    def set_joint_cmd(self, msg):
        if msg.width*3 > self.image_width or msg.height*3 > self.image_height:
            pass

        rospy.loginfo("Getting new ROI at x="+str(msg.x_offset)+" and y="+str(msg.y_offset))
        ...
        try:
            ...
            percent_offset_x = 0
            percent_offset_y = 0
        rospy.loginfo("percent_offset_x="+str(percent_offset_x)+" percent_offset_y="+str(percent_offset_y))
        ...
        if abs(percent_offset_x) > self.pan_threshold:
            ...
            rospy.loginfo("self.pan_speed="+str( self.pan_speed))
            ...
            rospy.loginfo("self.pan_position="+str( self.pan_position))
        else:
            self.pan_speed = 0
